...
void ai_mode(Fl_Widget *w, void *data) {
    ...
    ChessBoard *board = new ChessBoard(0, 0, window->w(), window->h(), 2, color);
...
void start_play_with_ai() {
    ...
    Fl_Button *start_button1 = new Fl_Button((window->w() - ButtonWidth) / 2, window->h() / 3, ButtonWidth, ButtonHeight, "Start Game as Black");
    start_button1->callback(ai_mode, (void *)color_ptr1);
    start_button1->labelsize(25);
    Fl_Button *start_button2 = new Fl_Button((window->w() - ButtonWidth) / 2, window->h() * 2 / 3, ButtonWidth, ButtonHeight, "Start Game as White");
